...
from vulcan.build_backend import add_requirement, build_editable, pack, unpack
...
class TestFixturesOutput:
    ...
    def test_dependencies_key_forbidden(
        self,
...
class TestConfig:
    ...
    def test_pep508_dict_extras(self) -> None:
        assert (
            to_pep508("somelib", {"version": "~=1.2.3", "extras": ["extra1", "extra2"]})
    ...
    @pytest.mark.parametrize(
        "mdata_file",
        ["METADATA", "entry_points.txt", "RECORD", "top_level.txt", "WHEEL"],
    )
    def test_metadata_files_exist_not_empty(self, test_built_application_wheel: Path, mdata_file: str) -> None:
...
class TestEditable:
    ...
    def test_built_editable_application_wheel(self, test_application: Path, tmp_path: Path) -> Path:
    ...
    def test_pack_unpack_idempotent(self, test_built_editable_application_wheel: Path, tmp_path: Path) -> None:
        (tmp_path / "tmp").mkdir()
