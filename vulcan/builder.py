...
def resolve_deps(install_requires: List[str], extras: Dict[str, List[str]]
    ...
    with builder as pipenv:  # type: ignore
        ...
        base_resolved = get_freeze(pipenv, site_packages)
        if not extras_list:
            # if we have no extras, we are done here.
            return sorted([str(req) for req in base_resolved.values()]), {}
        ...
        resolved_extra = get_freeze(pipenv, site_packages)
        resolved_extras = {extra: sorted([str(req) for req in resolved_extra.values()])}
        if len(extras_list) == 1:
            # if we have exactly 1 extra, we can get away with only using 1 venv in total
            return sorted([str(resolved_extra[req]) for req in base_resolved.keys()]), resolved_extras
    ...
    for extra, extra_reqs in extras_list[1:]:
        with builder as pipenv:  # type: ignore
            ...
            freeze = get_freeze(pipenv, site_packages)
            resolved_extras[extra] = sorted([str(all_resolved[req]) for req in freeze])

    return sorted([str(all_resolved[req]) for req in base_resolved.keys()]), resolved_extras
